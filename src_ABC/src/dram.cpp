...
DRAM *dram_new()
{
    ...
    DRAM* dram = (DRAM*)calloc(1, sizeof(DRAM)); // Memory allocation for structure dram
    assert(NUM_BANKS <= MAX_DRAM_BANKS);
    return dram;
}
...
uint64_t dram_access(DRAM *dram, uint64_t line_addr, bool is_dram_write)
{
    ...
    unsigned int delay = DELAY_SIM_MODE_B;
    // Check for SIM_MODE
    if (SIM_MODE == SIM_MODE_C) // || (SIM_MODE == SIM_MODE_DEF)) --> uncomment for parts d,e,f
    {
                               ...
                               bool is_dram_write)
{
    ...
    unsigned int bank_id_num = (line_addr / (ROW_BUFFER_SIZE / CACHE_LINESIZE)) & (NUM_BANKS - 1);
    // Address of row in bank
    unsigned int row_num_bank = (line_addr) / (ROW_BUFFER_SIZE / CACHE_LINESIZE) / (NUM_BANKS);
    delay = delay + DELAY_CAS + DELAY_BUS;
    // Check if row buffer hit/miss, true --> hit, false --> miss
    if (dram->row_buffer_per_bank[bank_id_num].valid == true)
    {
        //If row buffer hit and no conflict, do nothing
        if (dram->row_buffer_per_bank[bank_id_num].row_num == row_num_bank)
        {
        }
        // if row buffer conflict, bring the required row to the row buffer and calculate delay
        else
        {
            dram->row_buffer_per_bank[bank_id_num].row_num = row_num_bank;
            delay = delay + DELAY_ACT + DELAY_PRE;
        }
    }
    // If row buffer miss
    else
    {
        dram->row_buffer_per_bank[bank_id_num].valid = true;
        dram->row_buffer_per_bank[bank_id_num].row_num = row_num_bank;
        delay = delay + DELAY_ACT;
    }
    return delay;
}
