...
{
    bool dcache_access_wanted = false;
    bool is_write = false;
...
uint64_t memsys_access_modeBC(MemorySystem *sys, uint64_t line_addr,
                              AccessType type, unsigned int core_id)
{
    //The delay in cycles incurred by this memory access
    uint64_t delay = 0;
    unsigned int temp_delay = 0;
    unsigned int check_index;
    unsigned int evicted_line_id;
    bool is_write = false;
    CacheResult result;
    bool dcache_access_wanted = false;
    ...
    if (type == ACCESS_TYPE_IFETCH)
    {
        ...
        // Access icache
        result = cache_access(sys->icache, line_addr, is_write, core_id);
        if (result == MISS)
        {
            //icache miss
            cache_install(sys->icache, line_addr, is_write, core_id);
            delay = delay + memsys_l2_access(sys, line_addr, false, core_id); //L2 read access
        }
        else if (result == HIT)
        {
    ...
    if (dcache_access_wanted)
    {
        // function call for cache_access --> dcache access
        result = cache_access(sys->dcache, line_addr, is_write, core_id);
        // if dcache miss
        if (result == MISS)
        {
            cache_install(sys->dcache, line_addr, is_write, core_id);
            delay = delay + memsys_l2_access(sys, line_addr, false, core_id); //access L2 read
            // if dcache entry is dirty
            if (sys->dcache->last_evicted_line.is_dirty == true) 
            {
                // and dirty evict is valid
                if (sys->dcache->last_evicted_line.is_valid == true)
                {
                    // write back to L2 cache
                    check_index = line_addr % (sys->dcache->number_of_sets);
                    evicted_line_id = sys->dcache->last_evicted_line.tag * sys->dcache->number_of_sets + check_index;
                    // access L2 cache
                    temp_delay = temp_delay + memsys_l2_access(sys, evicted_line_id, true, core_id);
                    sys->dcache->last_evicted_line.is_dirty = false;
                    sys->dcache->last_evicted_line.is_valid = false;
                }
            }
        }
    }
                          ...
                          bool is_writeback, unsigned int core_id)
{
    ...
    CacheResult result = cache_access(sys->l2cache, line_addr, is_writeback, core_id);
    ...
    if (result == MISS)
    {
        ...
        // access dram read
        delay = delay + dram_access(sys->dram, line_addr, false);
        // if dirty evict is valid
        if (sys->l2cache->last_evicted_line.is_valid == true)
        {
            // and L2 entry is dirty
            if (sys->l2cache->last_evicted_line.is_dirty == true)
            {
                //write back to dram
                check_index = line_addr % (sys->l2cache->number_of_sets);
                evicted_line_id = sys->l2cache->last_evicted_line.tag * sys->l2cache->number_of_sets + check_index;
                temp_delay = temp_delay + dram_access(sys->dram, evicted_line_id, true); // access dram write
                sys->l2cache->last_evicted_line.is_valid = false;
                sys->l2cache->last_evicted_line.is_dirty = false;
            }
                               ...
                               AccessType type, unsigned int core_id)
{
    ...
    if (type == ACCESS_TYPE_IFETCH)
    {
        // TODO: Simulate the instruction fetch and update delay accordingly.
        //Access icache
        //result = cache_access(sys->icache_coreid[core_id], p_line_addr, is_write, core_id);
        //if (result == HIT)
        //{
            delay = delay + ICACHE_HIT_LATENCY;
        //}
        ...
        //    delay = delay + memsys_l2_access(sys, p_line_addr, false, core_id); //L2 read access
        //}
    }
    ...
    if (type == ACCESS_TYPE_LOAD)
    {
        ...
        //else if (result == MISS)
        //{
        //    // function call for dcache access
        //    result = cache_access(sys->dcache_coreid[core_id], p_line_addr, is_write, core_id);
        //    cache_install(sys->dcache_coreid[core_id], p_line_addr, is_write, core_id);
        //    delay = delay + memsys_l2_access(sys, p_line_addr, false, core_id); //access L2 read
        //    // if dcache entry is dirty
        //    if (sys->dcache_coreid[core_id]->last_evicted_line.is_dirty == true)
        //    {
        //        // and dirty evict is valid
        //        if (sys->dcache_coreid[core_id]->last_evicted_line.is_valid == true)
        //        {
        //            // write back to L2 cache
        //            check_index = p_line_addr % (sys->dcache_coreid[core_id]->number_of_sets);
        //            evicted_line_id = sys->dcache_coreid[core_id]->last_evicted_line.tag * sys->dcache_coreid[core_id]->number_of_sets + check_index;
        //            // access L2 cache
        //            temp_delay = temp_delay + memsys_l2_access(sys, evicted_line_id, true, core_id);
        //            sys->dcache_coreid[core_id]->last_evicted_line.is_dirty = false;
        //            sys->dcache_coreid[core_id]->last_evicted_line.is_valid = false;
        //        }
        //    }
        //}
    }
...
{
    assert(NUM_CORES == 2); 
    unsigned int tail_bit = vpn & 0x000fffff;
    unsigned int head_bit = vpn >> 20;
    unsigned int pfn = tail_bit + (core_id << 21) + (head_bit << 21);
    return pfn;
}
